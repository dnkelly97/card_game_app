  ...
  def create
    if session_params.has_key?(:password) && session_params.has_key?(:email)
      ...
      if @current_user&.authenticate(session_params[:password])
        ...
        redirect_to dashboard_path, flash: { notice: "Welcome #{@current_user.user_id}. Your account was successfully logged in." }
        return
      end
    end
    redirect_to login_path, flash: { notice: 'Unsuccessful login' }
  end
